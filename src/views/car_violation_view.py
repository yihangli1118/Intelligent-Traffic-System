# file: src/views/car_violation_view.py
...
class CarViolationView:
    def __init__(self, ui):
        ...
        # 初始化视频显示标签
        self.original_video_label = QLabel(self.ui.vedio_ori_car)
        self.original_video_label.setAlignment(Qt.AlignCenter)
        self.original_video_label.setGeometry(self.ui.vedio_ori_car.rect())

        self.processed_video_label = QLabel(self.ui.vedio_pro_car)
        self.processed_video_label.setAlignment(Qt.AlignCenter)
        self.processed_video_label.setGeometry(self.ui.vedio_pro_car.rect())

        # 连接按钮事件
        self.ui.car_con.clicked.connect(self.toggle_play_pause)
    ...
    def load_videos(self):
        ...
        """
        # 加载原始视频
        self.original_video_capture = cv2.VideoCapture('videos/car.mp4')
        # 加载处理后的视频
        self.processed_video_capture = cv2.VideoCapture('videos/car_result.mp4')

        # 检查视频是否成功加载
        if not self.original_video_capture.isOpened():
            print("无法打开原始视频文件: videos/car.mp4")
            return False

        if not self.processed_video_capture.isOpened():
            print("无法打开处理后的视频文件: videos/car_result.mp4")
            return False
