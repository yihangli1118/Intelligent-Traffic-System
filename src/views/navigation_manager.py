...
class NavigationManager:
    ...
    def setup_connections(self):
        ...
        self.ui.vio_side.clicked.connect(self.switch_to_vio_page)

        # 连接子页面切换按钮
        self.ui.violations.clicked.connect(self.switch_to_violations_page)
        self.ui.flow.clicked.connect(self.switch_to_flow_page)

        self.ui.red_car.clicked.connect(self.switch_to_red_car_page)
        self.ui.red_person.clicked.connect(self.switch_to_red_person_page)
    ...
    def switch_to_vio_page(self):
        """
        切换到违规检测主页面 (stackedWidget索引5，vio界面)
        """
        # 取消其他主页面按钮的选中状态
        self.ui.monitor.setChecked(False)
        self.ui.infoQuery.setChecked(False)
        ...
        # 设置当前按钮为选中状态
        self.ui.vio_side.setChecked(True)

        self.ui.stackedWidget.setCurrentIndex(4)
        # 重置信息查询的子按钮状态
        self.reset_info_query_sub_buttons()

    def switch_to_red_car_page(self):
        """
        切换到车辆闯红灯页面 (stackedWidget_3索引0，car_widget界面)
        """
        # 取消red_person按钮的选中状态
        self.ui.red_person.setChecked(False)

        # 设置当前按钮为选中状态
        self.ui.red_car.setChecked(True)

        self.ui.stackedWidget_3.setCurrentIndex(0)

    def switch_to_red_person_page(self):
        """
        切换到行人闯红灯页面 (stackedWidget_3索引1，person_widget界面)
        """
        # 取消red_car按钮的选中状态
        self.ui.red_car.setChecked(False)

        # 设置当前按钮为选中状态
        self.ui.red_person.setChecked(True)
